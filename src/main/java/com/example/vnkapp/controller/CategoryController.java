...
public class CategoryController {
    ...
    public ResponseEntity<?> createCategory(@Valid @RequestBody CategoryCreateRequestDto request) {
        log.info("Create category request: {}", request.name());
        try {
            CategoryResponseDto category = categoryService.createCategory(request);
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(new ApiResponseDto<>("Ok", null, category));
        } catch (IllegalArgumentException ex) {
            log.warn("Create category failed: {}", ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new UserResponseDto(null, ex.getMessage()));
        } catch (Exception ex) {
            log.error("Create category error for: {}", request.name(), ex);
    ...
    public ResponseEntity<?> updateCategory(
        ...
        try {
            ...
            log.warn("Update category {} failed: {}", id, ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new UserResponseDto(null, ex.getMessage()));
        } catch (Exception ex) {
            log.error("Update category {} error", id, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> deleteCategory(@PathVariable UUID id) {
        ...
        try {
            ...
            log.warn("Delete category {} failed: {}", id, ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new UserResponseDto(null, ex.getMessage()));
        } catch (Exception ex) {
            log.error("Delete category {} error", id, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> getCategory(@PathVariable UUID id) {
        ...
        try {
            ...
            log.error("Get category {} error", id, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> getAllCategories() {
        ...
        try {
            ...
            log.error("Get all categories error", ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> getSubcategories(@PathVariable UUID id) {
        log.info("Get subcategories for category: {}", id);
        try {
            ...
            log.error("Get subcategories error for category: {}", id, ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
